...
for(int i=0;i<subMatTestPcaOcr.rows();i++){
	...
	if(resultSVM==answer[i]){
		...
		System.out.println("預測錯誤!支援向量機預測是"+resultSVM+",正確是="+answer[i]);
	}
   //////////////////////////////////////////
...
System.out.println("///////////////////////////////////////////////////////////");
System.out.println("支援向量機測試精準值="+((float)rightSVM/(float)subMatTestPcaOcr.rows())*100+"%");
System.out.println("KNN測試精準值="+((float)rightKnn/(float)subMatTestPcaOcr.rows())*100+"%");
	}
