...
public class Ch13_10_5AnnForOnlineOCR {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch13_10_5AnnForOnlineOCR window = new Ch13_10_5AnnForOnlineOCR();
	 ...
	 */
	public Ch13_10_5AnnForOnlineOCR() {
		initialize();
	}
	...
	private void initialize() {
		...
		final CvANN_MLP ann=new CvANN_MLP();
		
		Mat layerSize=new Mat(3,1,CvType.CV_32SC1);
		int[] layerSizeAry={144, 10, 10};
		...
		JButton btnMLpredict = new JButton("類神經網路預測");
...
//				System.out.println(t.dump());//ok
				 ...
				 ann.predict(ocr.handleForML(getGlobalMat()),results);
				 MinMaxLocResult minMaxLocResult=Core.minMaxLoc(results);
				 System.out.println("預測結果="+minMaxLocResult.maxLoc.x);
				 lblMLresult.setText("預測結果="+minMaxLocResult.maxLoc.x);
	...
	public Mat paint(Mat src,int x,int y){
		Core.circle(src, new Point(x,y), 2, new Scalar(0,0,0),2);
