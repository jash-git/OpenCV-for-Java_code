...
public class Ch10_5_2FitEllipse {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch10_5_2FitEllipse window = new Ch10_5_2FitEllipse();
	...
	private void initialize() {
		...
		frmjavaSwing.setTitle("opencv 尋找輪廓並且繪出圓形2");
	...
	public Mat FindAndDrawContours(double threshold1){
		Mat source = Imgcodecs.imread("C://opencv3.1//samples//stuff.jpg",Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE); 
		Mat destination = new Mat(source.rows(),source.cols(),source.type());
         ...
         for(int i=0;i<contours.size();i++){
        	 ...
        	 if( contours.get(i).toArray().length > 5 ){
        		 ...
        		 Imgproc.ellipse(drawing, r, new Scalar(0,0,255),3,8);
        	 }
