	...
	public static void main(String arg[]) {
		...
		if (!capture.isOpened()) {
			...
			frame1.setSize(webcam_image.width() + 10,webcam_image.height() + 20);
			 ...
			 CascadeClassifier eyeDetector = new CascadeClassifier("C://opencv3.1//sources//data//haarcascades//haarcascade_mcs_eyepair_big.xml");
			 CascadeClassifier noseDetector = new CascadeClassifier("C://opencv3.1//sources//data//haarcascades//haarcascade_mcs_nose.xml");
			...
			while (true) {
				...
				if( !webcam_image.empty() ){
		           ...
		           {  
		                ...
		                Imgproc.rectangle(webcam_image, center, new Point(rect.x + rect.width*0.5, rect.y + rect.height*0.5 ),  new Scalar( 105, 250, 105 ) );  
		           }  
					 
				        System.out.println("Detected "+ faceDetections.toArray().length+ "faces,"+eyeDetections.toArray().length+ "eye,"+noseDetections.toArray().length+" nose");
