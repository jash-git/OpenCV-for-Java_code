...
public class Ch08_13_1LogPolar {
	...
	private void initialize() {
		...
		slider_flag.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(LinearPolar(slider_M.getValue(),slider_flag.getValue()));
	...
	public Mat LinearPolar(double M,int flag){
		...
		Mat source = Imgcodecs.imread("C://opencv3.1//samples//linearPolar_test.jpg");
		 Mat dst=new Mat();
		 Imgproc.logPolar(source, dst, new Point(source.cols()/2,source.rows()/2), M, flag);
		return dst;
