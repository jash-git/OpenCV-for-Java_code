...
public class Ch09_7_1TextureFlattening {
	...
	private void initialize() {
		...
		slider_ksize.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(TextureFlattening(slider_lowT.getValue(),slider_highT.getValue(),slider_ksize.getValue()));
	...
	public Mat TextureFlattening(float low_threshold,float high_threshold,int kernel_size){
		System.out.println("low_threshold="+low_threshold+",high_threshold="+high_threshold+",kernel_size="+kernel_size);
		Mat src = Imgcodecs.imread("C://opencv3.1//samples//DSC_0679.jpg",Imgcodecs.CV_LOAD_IMAGE_COLOR);
	    ...
	    Photo.textureFlattening(src,src_mask, destination, low_threshold, high_threshold, kernel_size);
		return destination;
