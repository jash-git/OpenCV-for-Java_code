	     ...
	     {  
	      ...
	      {  
	        ...
	        if( !webcam_image.empty() )  
	         {  
	        	 Mat distance=new Mat(webcam_image.height(),webcam_image.width(),Highgui.CV_LOAD_IMAGE_GRAYSCALE);  
	        	   Imgproc.GaussianBlur(webcam_image, distance,new Size(9,9),20,0);
	        	   Imgproc.Canny(distance, distance, 100, 120);
	        	   Mat circles = new Mat();
	        	   Imgproc.HoughCircles(distance, circles, Imgproc.CV_HOUGH_GRADIENT, 1.3, distance.height()/2, 500.0, 50.0,0,0);
	             ...
	             //畫出所有圓
	               if(circles.cols()>0){
	      	         ...
	      	         for(int i=0;i<circles.cols();i++){
	      	        	...
	      	        	Core.putText(webcam_image,"("+center.x+","+center.y+")",new Point(center.x, center.y) , 1  ,1.0,new Scalar(100,10,10,255),1);
	               ...
	               for(int i=0;i<fingerPoint.size();i++)
		        	...
		        	if(i>0){
		        		Core.line(webcam_image, (Point)fingerPoint.get(i), (Point)fingerPoint.get(i), new Scalar(ran.nextInt(255)+1, ran.nextInt(255)+1, ran.nextInt(255)+1), ran.nextInt(12)+5);
		        	}
