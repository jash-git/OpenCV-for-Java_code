	...
	public static void main(String arg[]) {
		...
		if (!capture.isOpened()) {
			...
			frame1.setSize(webcam_image.width() + 10,webcam_image.height() + 20);
			 ...
			 CascadeClassifier pedestriansDetector = new CascadeClassifier("C://opencv3.1//sources//data//hogcascades//hogcascade_pedestrians.xml");
				...
				if( !webcam_image.empty() ){
					pedestriansDetector.detectMultiScale(webcam_image, pedestriansDetections);
					 for (Rect rect : pedestriansDetections.toArray()) {
				        ...
				        System.out.println(String.format("Detected %s pedestrians", pedestriansDetections.toArray().length));
