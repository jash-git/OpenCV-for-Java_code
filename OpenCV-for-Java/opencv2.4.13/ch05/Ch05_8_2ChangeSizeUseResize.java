...
public class Ch05_8_2ChangeSizeUseResize {
	...
	private void initialize() {
		...
		slider.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage( resize(getSource(),1,slider.getValue()));
			}
		});
		
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				BufferedImage newImage=matToBufferedImage( resize(getSource(),0.5,slider.getValue()));
				lblNewLabel.setIcon(new ImageIcon(newImage));
			}
		});
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				BufferedImage newImage=matToBufferedImage( resize(getSource(),2,slider.getValue()));
				lblNewLabel.setIcon(new ImageIcon(newImage));
			}
		});
	}
	public Mat resize(Mat source,double size ,int interpolation){
		 ...
		 Imgproc.resize(source, destination,destination.size(), size,size,interpolation);
