...
public class Ch09_12_1Stylization {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch09_12_1Stylization window = new Ch09_12_1Stylization();
	...
	private void initialize() {
		final Mat source = Imgcodecs.imread("C://opencv3.1//samples//DSC_0718.jpg");
		
		BufferedImage image=matToBufferedImage(source);
		
		frmjavaSwing = new JFrame();
		frmjavaSwing.setTitle("opencv stylization ³B²z½m²ß");
		frmjavaSwing.setBounds(100, 100, 520, 448);
		frmjavaSwing.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frmjavaSwing.getContentPane().setLayout(null);
		
		final JLabel showRedValue = new JLabel("1");
		showRedValue.setBounds(120, 43, 46, 15);
		frmjavaSwing.getContentPane().add(showRedValue);
		
		final JLabel showGreenValue = new JLabel("0.1");
		showGreenValue.setBounds(371, 43, 46, 15);
		frmjavaSwing.getContentPane().add(showGreenValue);
		
		final JLabel lblNewLabel = new JLabel("");
		lblNewLabel.setBounds(10, 68, 473, 327);
		...
		slider_sigmaS.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(Stylization((float)slider_sigmaS.getValue(),(float)slider_sigmaR.getValue()/10));
	...
	public Mat Stylization(float sigma_s,float sigma_r){
		Mat src = Imgcodecs.imread("C://opencv3.1//samples//DSC_0718.jpg",Imgcodecs.CV_LOAD_IMAGE_COLOR);
		Mat destination=new Mat();
		
	    Photo.stylization(src,destination,sigma_s,sigma_r);
		return destination;
	}
