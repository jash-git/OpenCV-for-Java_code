		...
		if (!capture.isOpened()) {
			...
			while (true) {
				...
				lblNewLabel.setIcon(new ImageIcon(newImage));
				  ...
				  Imgproc.circle(hsv_image, new Point(hsv_image.rows()/2,hsv_image.cols()/2), 10, new Scalar(0,0,0),3);  
				  Imgproc.line(hsv_image, new Point(hsv_image.rows()/2,hsv_image.cols()/2-10), new Point(hsv_image.rows()/2,hsv_image.cols()/2-70), new Scalar(0,0,0),3);
				  Imgproc.line(hsv_image, new Point(hsv_image.rows()/2,hsv_image.cols()/2-70), new Point(hsv_image.rows()/2+150,hsv_image.cols()/2-70), new Scalar(0,0,0),3);
