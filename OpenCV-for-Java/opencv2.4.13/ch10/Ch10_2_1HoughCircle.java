...
public class Ch10_2_1HoughCircle {
	...
	public Ch10_2_1HoughCircle() {
		...
		System.out.println("Imgproc.CV_HOUGH_MULTI_SCALE="+Imgproc.CV_HOUGH_MULTI_SCALE);
		System.out.println("Imgproc.CV_HOUGH_PROBABILISTIC="+Imgproc.CV_HOUGH_PROBABILISTIC);
	...
	private void initialize() {
		final Mat source = Highgui.imread("C://opencv3.1//samples//coin.jpg");
		...
		slider_minR.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(HoughCircle(slider_GBK.getValue(),slider_cth1.getValue(),slider_method.getValue(),slider_dp.getValue(),slider_minDist.getValue(),slider_param1.getValue(),slider_param2.getValue(),slider_minR.getValue(),slider_maxR.getValue()));
	...
	public Mat HoughCircle(double GBK, double threshold1,  int method, double dp, double minDist, double param1,double param2, int minRadius, int maxRadius){
		Mat source1 = Highgui.imread("C://opencv3.1//samples//coin.jpg"); 
		Mat source = Highgui.imread("C://opencv3.1//samples//coin.jpg",Highgui.CV_LOAD_IMAGE_GRAYSCALE); 
		Mat destination = new Mat(source.rows(),source.cols(),source.type());
		 Imgproc.GaussianBlur(source, source,new Size(GBK,GBK),20,0);
         Imgproc.Canny(source,destination, threshold1, 100);
		 Mat circles = new Mat();
		  Imgproc.HoughCircles(destination, circles, method, dp, destination.height()/minDist, param1, param2,minRadius,maxRadius);
         ...
         if(circles.cols()>0){
	         ...
	         for(int i=0;i<circles.cols();i++){
	        	...
	        	Core.circle(source1, center, 3, new Scalar(255,0,0), 3, 8, 0 );
	        	
	        	//draw the circle outline
	        	Core.circle(source1, center, radius, new Scalar(255,0,255), 4, 8, 0 );
	        }
