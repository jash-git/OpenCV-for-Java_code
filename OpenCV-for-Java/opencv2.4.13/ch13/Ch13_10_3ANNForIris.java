...
public class Ch13_10_3ANNForIris {
		...
		for(int i=0;i<iris.getTestingDataMat().rows();i++){
		 ...
		 minMaxLocResult=Core.minMaxLoc(results);
			if(minMaxLocResult.maxLoc.x==answer[i]){
				...
				System.out.println("預測錯誤!類神經網路預測是"+minMaxLocResult.maxLoc.x+"正確是="+answer[i]);
			}

		}
		System.out.println("類神經網路測試精準值="+((float)right/(float)iris.getTestingDataMat().rows())*100+"%");
