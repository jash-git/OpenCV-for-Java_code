			...
			for (int i = 0; i < contours.size(); i++) {
				...
				for (int j = 0; j < hull.size().height - 1; j++) {
					...
					pt = new Point(tempContour.get(index, 0)[0],
							tempContour.get(index, 0)[1]);
					Core.line(source, pt0, pt, new Scalar(255, 0, 100), 2);// 2是厚度,藍,凸包的輪廓
				...
				for (int j = 0; j < mConvexityDefectsMatOfInt4.size().height - 1; j++) {
					...
					Core.line(source, pt1, pt1, new Scalar(0, 255, 0), 9);// 尖dot,綠
					Core.line(source, pt3, pt3, new Scalar(150, 155, 255), 12);// 底dot,粉紅
					Core.line(source, pt1, pt3, new Scalar(0, 0, 255), 2);// 紅,姆指左
					if (pt4 != null && old_pt1 != null) {
						Core.line(source, pt1, pt4, new Scalar(200, 0, 200), 2);// 桃紅,姆指右
