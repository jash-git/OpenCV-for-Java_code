...
public class Ch10_6_1PointPolygonTest {
	...
	private void initialize() {
		final Mat source = Highgui.imread("C://opencv3.1//samples//shape-sample.jpg");
		String showData="";
		...
		lblNewLabel.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				BufferedImage newImage=matToBufferedImage(isInner(1,arg0.getX(),arg0.getY()));
				lblNewLabel.setIcon(new ImageIcon(newImage));
				int simpleDist=(int) dist;
				if(simpleDist>0){
				Label_click.setText("點擊於X:"+arg0.getX()+",Y:"+arg0.getY()+"在區域內,距離:"+simpleDist);
				}else if(simpleDist<0){
					Label_click.setText("點擊於X:"+arg0.getX()+",Y:"+arg0.getY()+"在區域外,距離:"+(0-simpleDist));
				}else if(simpleDist==0){
					Label_click.setText("點擊於X:"+arg0.getX()+",Y:"+arg0.getY()+"在輪廓上");
				}
			}
		});
	}
	...
	public Mat isInner(double threshold1,int X,int Y){
		Mat source = Highgui.imread("C://opencv3.1//samples//shape-sample.jpg",Highgui.CV_LOAD_IMAGE_GRAYSCALE); 
		//Mat source = Highgui.imread("C://opencv249//sources//samples//c//airplane.jpg",Highgui.CV_LOAD_IMAGE_GRAYSCALE); 
		Mat destination = new Mat(source.rows(),source.cols(),source.type());
         ...
         dist= Imgproc.pointPolygonTest(new MatOfPoint2f(contours.get(0).toArray()), new Point(X,Y), true);
