...
public class Ch06_4_3SobelFilter {
	...
	private void initialize() {
		...
		slider_Ksize.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(Sobel(source,slider_dx.getValue(),slider_dy.getValue(),slider_Ksize.getValue(),slider_Scale.getValue(),slider_Delta.getValue()));
	...
	public Mat Sobel(Mat source,int dx,int dy,int ksize,double scale,double delta){
		 if((dx==0)&&(dy==0)){
			 dx=1;
		 }
