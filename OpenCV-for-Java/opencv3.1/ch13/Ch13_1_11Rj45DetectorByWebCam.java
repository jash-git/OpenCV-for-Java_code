	...
	public static void main(String arg[]) {
		...
		if (!capture.isOpened()) {
			...
			frame1.setSize(webcam_image.width() + 10,webcam_image.height() + 20);
			 ...
			 CascadeClassifier rj45Detector = new CascadeClassifier("C://opencv3.1//samples//rj45cascade.xml");
			...
			while (true) {
				...
				if( !webcam_image.empty() ){
					 ...
					 for (Rect rect : rj45Detections.toArray()) {
						 ...
						 Imgproc.ellipse( webcam_image, center, new Size( rect.width*0.5, rect.height*0.5), 0, 0, 360, new Scalar( 255, 0, 255 ), 4, 8, 0 );  
				        }
					
				        System.out.println(String.format("Detected %s rj45s", rj45Detections.toArray().length));
