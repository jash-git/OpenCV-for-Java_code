...
public class Ch10_8_1FindTemplete {
	...
	private void initialize() {
		...
		final Mat template = Highgui.imread("C://opencv3.1//samples//goat-template1.jpg");
		...
		lblNewLabel_2.setIcon(new ImageIcon("C://opencv3.1//samples//goat-template1.jpg"));
		...
		slider_method.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				Mat src = Highgui.imread("C://opencv3.1//samples//DSC_0864.jpg");
	...
	public Mat findTemplete(Mat src,Mat template, int match_method){
	    ...
	    Core.rectangle(src, matchLoc, new Point(matchLoc.x + template.cols(),
	            matchLoc.y + template.rows()), new Scalar(0, 255, 0));
	    
	    Core.rectangle(result, matchLoc, new Point(matchLoc.x + template.cols(),
	            matchLoc.y + template.rows()), new Scalar(0, 255, 0));
	    Highgui.imwrite("C://opencv3.1//samples//matchTemplate-result.jpg", result);
