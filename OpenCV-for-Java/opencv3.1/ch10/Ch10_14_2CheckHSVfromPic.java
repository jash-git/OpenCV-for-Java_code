...
public class Ch10_14_2CheckHSVfromPic {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch10_14_2CheckHSVfromPic window = new Ch10_14_2CheckHSVfromPic();
	 ...
	 */
	public Ch10_14_2CheckHSVfromPic() {
		initialize();
	}
	...
	private void initialize() {
		final Mat source = Imgcodecs.imread("C://opencv3.1//samples//DSC_0691.jpg");
		...
		lblNewLabel.setBounds(10, 68, 564, 387);
		...
		slider_Hmin.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(matConvertTo(slider_Hmin.getValue(),slider_Hmax.getValue(),slider_Smin.getValue(),slider_Smax.getValue(),slider_Vmin.getValue(),slider_Vmax.getValue()));
	...
	public Mat matConvertTo(double Hmin,double Hmax,double Smin,double Smax,double Vmin,double Vmax){
		 Mat source2 = Imgcodecs.imread("C://opencv3.1//samples//DSC_0691.jpg");
		 Mat hsv_image=new Mat();
         Imgproc.GaussianBlur(source2, source2, new Size(3,3),0,0); 
         Imgproc.cvtColor(source2, hsv_image, Imgproc.COLOR_BGR2HSV);  
         Mat thresholded=new Mat();
         Core.inRange(hsv_image, new Scalar(Hmin,Smin,Vmin), new Scalar( Hmax,Smax,Vmax), thresholded);
	        return thresholded;
