...
public class Ch11_7_1CornerEigenValsAndCornerMinEigenVal {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch11_7_1CornerEigenValsAndCornerMinEigenVal window = new Ch11_7_1CornerEigenValsAndCornerMinEigenVal();
	...
	private void initialize() {
		...
		slider_myShi.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(myShiTomasi(slider_myShi.getValue()));
		...
		slider_myHarris.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(myHarris(slider_myHarris.getValue()));
				lblNewLabel.setIcon(new ImageIcon(newImage));
			}
		});
	...
	public Mat myHarris(double myHarrisQualityLevel){
		...
		// 特徵點選擇  
				...
				// 角點滿足條件則繪製  
			     for( int j = 0; j < source.rows(); j++ )  {   
			    	   for( int i = 0; i < source.cols(); i++ )   {  
			    	               if( resHarris.get(j,i)[0] > HarrisMinVal + ( HarrisMaxVal - HarrisMinVal )*myHarrisQualityLevel/maxQualityLevel )  
		...
		Imgproc.cvtColor( source,sourceGray, Imgproc.COLOR_BGR2GRAY );  
		 Mat dstShiTomasi = Mat.zeros( source.size(), CvType.CV_32FC1 );    
		    ...
		    // 角點滿足條件則繪製  
		     for( int j = 0; j < source.rows(); j++ )  {   
		    	   for( int i = 0; i < source.cols(); i++ )   {  
		    	               if( dstShiTomasi.get(j,i)[0] > mmr.minVal + ( mmr.maxVal - mmr.minVal )*myShiTomasiQualityLevel/maxQualityLevel )  
