...
public class Ch10_1_3LineSegmentDetector {
	...
	public Mat LineSegmentDetector(int refine, float scale, float sigma_scale, float quant,float ang_th,int log_eps, float density_th, int n_bits){
		Mat source =Imgcodecs.imread("C://opencv3.1//samples//DSC_0763.jpg",Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
			Mat Line=new Mat();
			Mat draw=new Mat(source.size(),source.type());
			org.opencv.imgproc.LineSegmentDetector lsd=new org.opencv.imgproc.Imgproc().createLineSegmentDetector(refine, scale, sigma_scale, quant, ang_th, log_eps,  density_th,  n_bits);
