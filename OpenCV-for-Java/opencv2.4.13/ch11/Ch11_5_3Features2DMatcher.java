...
public class Ch11_5_3Features2DMatcher {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch11_5_3Features2DMatcher window = new Ch11_5_3Features2DMatcher();
	 ...
	 */
	public Ch11_5_3Features2DMatcher() {
		initialize();
		...
		System.out.println("DescriptorExtractor.OPPONENT_BRIEF="+DescriptorExtractor.OPPONENT_BRIEF);
		System.out.println("DescriptorExtractor.OPPONENT_BRISK="+DescriptorExtractor.OPPONENT_BRISK);
		System.out.println("DescriptorExtractor.OPPONENT_FREAK="+DescriptorExtractor.OPPONENT_FREAK);
		System.out.println("DescriptorExtractor.OPPONENT_ORB="+DescriptorExtractor.OPPONENT_ORB);
		System.out.println("DescriptorExtractor.OPPONENT_SIFT="+DescriptorExtractor.OPPONENT_SIFT);
		System.out.println("DescriptorExtractor.OPPONENT_SURF="+DescriptorExtractor.OPPONENT_SURF);
		
		System.out.println("--------------DescriptorMatcher-------------------");
		System.out.println("DescriptorMatcher.FLANNBASED="+DescriptorMatcher.FLANNBASED);
		System.out.println("DescriptorMatcher.BRUTEFORCE="+DescriptorMatcher.BRUTEFORCE);
		System.out.println("DescriptorMatcher.BRUTEFORCE_HAMMING="+DescriptorMatcher.BRUTEFORCE_HAMMING);
		System.out.println("DescriptorMatcher.BRUTEFORCE_HAMMINGLUT="+DescriptorMatcher.BRUTEFORCE_HAMMINGLUT);
		System.out.println("DescriptorMatcher.BRUTEFORCE_L1="+DescriptorMatcher.BRUTEFORCE_L1);
		System.out.println("DescriptorMatcher.BRUTEFORCE_SL2="+DescriptorMatcher.BRUTEFORCE_SL2);
	}
	...
	private void initialize() {
		final Mat source = Highgui.imread("C://opencv3.1//samples//DSC_0864.jpg",Highgui.CV_LOAD_IMAGE_COLOR);
		final Mat object1 = Highgui.imread("C://opencv3.1//samples//goat-template5.jpg",Highgui.CV_LOAD_IMAGE_COLOR);
		final Mat object2 = Highgui.imread("C://opencv3.1//samples//goat-template3.jpg",Highgui.CV_LOAD_IMAGE_COLOR);
		...
		slider.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				if(slider.getValue()<7){
					...
					newImage=matToBufferedImage(Features2D(object1,slider.getValue(),slider_1.getValue(),Integer.parseInt(Label_FeatureDetector.getText())));
				}else{
					lblNewLabel_2.setText((slider.getValue()-6+1000)+"");
					newImage=matToBufferedImage(Features2D(object1,(slider.getValue()-6+1000),slider_1.getValue(),Integer.parseInt(Label_FeatureDetector.getText())));
				}
		...
		slider_g2e.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				if(slider_g2e.getValue()<7){
					...
					newImage=matToBufferedImage(Features2D(object2,slider_g2e.getValue(),slider_g2m.getValue(),Integer.parseInt(Label_FeatureDetector.getText())));
				}else{
					label_g21.setText((slider_g2e.getValue()-6+1000)+"");
					newImage=matToBufferedImage(Features2D(object2,(slider_g2e.getValue()-6+1000),slider_g2m.getValue(),Integer.parseInt(Label_FeatureDetector.getText())));
				}
				lblNewLabel.setIcon(new ImageIcon(newImage));
			}
		});
	...
	public Mat Features2D(Mat object,int Extractor,int Matcher,int featureDetectorType){
		//尋找已知物,object
	    ...
	    FeatureDetector detector = FeatureDetector.create(featureDetectorType); // 4 = FeatureDetector.SURF
	    ...
	    for (int kk=0;kk<good_matches.size();kk++){
	    	System.out.println("Match-point-"+kk+",keypoint-queryIdx=" + good_matches.get(kk).queryIdx+",keypoint-trainIdx="+good_matches.get(kk).trainIdx+",keypoint-distance="+good_matches.get(kk).distance);
	    }
