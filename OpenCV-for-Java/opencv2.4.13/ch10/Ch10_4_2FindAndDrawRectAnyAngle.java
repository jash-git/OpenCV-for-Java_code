...
public class Ch10_4_2FindAndDrawRectAnyAngle {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch10_4_2FindAndDrawRectAnyAngle window = new Ch10_4_2FindAndDrawRectAnyAngle();
	 ...
	 */
	public Ch10_4_2FindAndDrawRectAnyAngle() {
		initialize();
	}
	...
	public Mat FindAndDrawContours(double threshold1){
		Mat source = Highgui.imread("C://opencv3.1//samples//stuff.jpg",Highgui.CV_LOAD_IMAGE_GRAYSCALE); 
		Mat destination = new Mat(source.rows(),source.cols(),source.type());
         ...
         for(int i=0;i<contours.size();i++){
        	 ...
        	 r.points(allPoint);
        	 for (int j = 0; j < 4; j++){
        		 Core.line(drawing, allPoint[j], allPoint[(j+1)%4], new Scalar(0,255,0),2);
        	 }
