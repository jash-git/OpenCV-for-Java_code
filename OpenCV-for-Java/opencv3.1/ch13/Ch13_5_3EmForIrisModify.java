...
IrisDatabase iris=new IrisDatabase();
 ...
 em.setCovarianceMatrixType(1);//// (covMatType == COV_MAT_SPHERICAL || covMatType == COV_MAT_DIAGONAL || covMatType == COV_MAT_GENERIC)
...
for(int i=0;i<iris.getTestingDataMat().rows();i++){
 ...
 result=em.predict(iris.getTestingDataMat().row(i));
	...
	if(result==answer[i]){
		...
		System.out.println("預測錯誤!最大期望值預測是"+result+"正確是="+answer[i]);
	}

}
System.out.println("最大期望值測試精準值="+((float)right/(float)iris.getTestingDataMat().rows())*100+"%");
