...
public class Ch11_3_1CornerSubPix {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch11_3_1CornerSubPix window = new Ch11_3_1CornerSubPix();
	...
	private void initialize() {
		...
		frmjavaSwing.setTitle("更精確的角點檢測方法");
		...
		slider_ql.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(cornerSubPix(slider_mc.getValue(),(float)slider_ql.getValue()/100,slider_md.getValue()));
	...
	public Mat cornerSubPix(int maxCorners, double qualityLevel, double minDistance){
		Mat source = Highgui.imread("C://opencv249//sources//samples//c//lena.jpg",Highgui.CV_LOAD_IMAGE_COLOR);
        ...
        Size zeroZone=new Size(-1,-1);
     	...
     	corners2f.fromList(corners.toList());
     	Imgproc.cornerSubPix(destination, corners2f, winSize, zeroZone, criteria);
     	
     	corners2f.convertTo(Pts, CvType.CV_32FC2);
     	AllPoint=Pts.toArray();
     	int count=1;
     	for(Point px: AllPoint){
     		System.out.println("精確Corner["+count+"]=("+px.x+","+px.y+")");
     		count++;
         }
		return source;
	}
