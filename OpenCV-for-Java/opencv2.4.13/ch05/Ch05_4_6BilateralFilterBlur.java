...
public class Ch05_4_6BilateralFilterBlur {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch05_4_6BilateralFilterBlur window = new Ch05_4_6BilateralFilterBlur();
	 ...
	 */
	public Ch05_4_6BilateralFilterBlur() {
		initialize();
	}
	...
	private void initialize() {
		...
		BufferedImage image=matToBufferedImage(BlurFilter(source,d));
		
		frmjavaSwing = new JFrame();
		frmjavaSwing.setTitle("bilateralFilter¼Ò½k½m²ß");
		...
		JLabel lblAlpha = new JLabel("blurring kernel size:");
		lblAlpha.setBounds(10, 10, 144, 15);
		frmjavaSwing.getContentPane().add(lblAlpha);
		
	}
	public Mat BlurFilter(Mat source,int d){
		
		 Mat destination=new Mat(source.rows(),source.cols(),source.type());
         Imgproc.bilateralFilter(source, destination, d,120,120);
		return destination;
