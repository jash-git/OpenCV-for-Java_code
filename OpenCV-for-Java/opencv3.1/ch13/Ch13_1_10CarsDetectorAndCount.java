		...
		if (!vid.isOpened()) {
			...
			while (true) {
				...
				if( !webcam_image.empty() ){
					...
					carsDetector.detectMultiScale3(isCarInROI, carsDetections, rejectLevels, levelWeights, scaleFactor, minNeighbors, flags, minSize, maxSize, outputRejectLevels);
					 for (Rect rect : carsDetections.toArray()) {
				        	Imgproc.rectangle(isCarInROI, new Point(rect.x, rect.y), new Point(rect.x + rect.width, rect.y + rect.height),
				     ...
				     //繪出ROI
				        ...
				        Imgproc.rectangle(webcam_image, new Point(0,40), new Point(235,130),new Scalar(255, 0, 0),1);
