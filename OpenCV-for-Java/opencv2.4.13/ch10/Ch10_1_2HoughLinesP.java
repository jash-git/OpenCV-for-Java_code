...
public class Ch10_1_2HoughLinesP {
	...
	private void initialize() {
		...
		slider_shape.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(HoughLine(source,slider_ksize.getValue(),slider_shape.getValue(),slider_op.getValue(),slider_CT1.getValue(),slider_CT2.getValue()));
				lblNewLabel.setIcon(new ImageIcon(newImage));
			}
		});
	...
	public Mat HoughLine(Mat source,int threshold,double minLineLength,double maxLineGap,double Canny_threshold1,double Canny_threshold2){
		 Mat mRgba = new Mat();
