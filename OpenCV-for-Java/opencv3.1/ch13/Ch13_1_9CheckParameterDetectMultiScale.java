...
public class Ch13_1_9CheckParameterDetectMultiScale {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch13_1_9CheckParameterDetectMultiScale window = new Ch13_1_9CheckParameterDetectMultiScale();
	...
	private void initialize() {
		...
		slider_sigmaS.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(CheckParameter((float)slider_sigmaS.getValue(),(float)slider_sigmaR.getValue()));
	...
	public Mat CheckParameter(float minSz,float maxSz){
		...
		CascadeClassifier carsDetector = new CascadeClassifier("C://opencv3.1//samples//cars3.xml");
		 MatOfRect carsDetections = new MatOfRect();

		Mat src = Imgcodecs.imread("C://opencv3.1//samples//testCar1.jpg",Imgcodecs.CV_LOAD_IMAGE_COLOR);
		
		carsDetector.detectMultiScale3(src, carsDetections, rejectLevels, levelWeights, scaleFactor, minNeighbors, flags, minSize, maxSize, outputRejectLevels);
