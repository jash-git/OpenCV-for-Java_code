...
public class Ch10_18_2FindFingerByPalmCenter {  
...
//	          Imgproc.GaussianBlur(thresholded, thresholded, new Size(15,15), 0, 0);
	        	...
	        	Mat target = new Mat(webcam_image.size(),CvType.CV_8U);
	    		//轉灰階 
	    		Imgproc.cvtColor(webcam_image, target, Imgproc.COLOR_RGB2GRAY);
	    		 
	    		 //除噪
	    		 Imgproc.GaussianBlur(target, target, new Size(15,15), 0, 0);
	    		 Mat threshold_output=new Mat(webcam_image.rows(),webcam_image.cols(),webcam_image.type());
	      ...
	      //      int s = findBiggestContour(contours);
	          ...
	          //  int count=0;
	            ...
	            //System.out.println("destination1="+destination1.dump());
	     		int temp=0;
	     		//半徑
	     		int R=0;
	     		...
	     		MatOfPoint2f plamContour=new MatOfPoint2f(contours.get(0).toArray());
	     	if(plamContour!=null && plamContour.total()>0 ){
	     		
	     
	     		d=Imgproc.pointPolygonTest(plamContour, new Point(j,i), true);	
	     		...
	     		Core.circle(webcam_image, new Point(centerX,centerY), R, new Scalar(255, 255, 255));
               ...
               //繪出掌心置放區,以十字表示
                Core.line(webcam_image, new Point(130,320), new Point(180,320), new Scalar(250, 250, 250),2);//橫
                Core.line(webcam_image, new Point(155,295), new Point(155,345), new Scalar(250, 250, 250),2);//豎
                **/
	          //-- 5. Display the image  
