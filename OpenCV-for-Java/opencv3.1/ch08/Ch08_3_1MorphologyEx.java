...
public class Ch08_3_1MorphologyEx {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch08_3_1MorphologyEx window = new Ch08_3_1MorphologyEx();
	 ...
	 */
	public Ch08_3_1MorphologyEx() {
		initialize();
		System.out.println("Imgproc.MORPH_OPEN="+Imgproc.MORPH_OPEN);
		System.out.println("Imgproc.MORPH_CLOSE="+Imgproc.MORPH_CLOSE);
		System.out.println("Imgproc.MORPH_GRADIENT="+Imgproc.MORPH_GRADIENT);
		System.out.println("Imgproc.MORPH_TOPHAT="+Imgproc.MORPH_TOPHAT);
		System.out.println("Imgproc.MORPH_BLACKHAT="+Imgproc.MORPH_BLACKHAT);
	...
	private void initialize() {
		...
		slider_shape.setBounds(262, 10, 66, 25);
		...
		slider_ksize.setBounds(47, 10, 150, 25);
		...
		lblBeta.setBounds(221, 10, 46, 15);
		frmjavaSwing.getContentPane().add(lblBeta);
		
		
		final JLabel showOpValue = new JLabel("2");
		showOpValue.setBounds(432, 10, 46, 15);
		...
		slider_op.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(morphy(source,slider_ksize.getValue(),slider_shape.getValue(),slider_op.getValue()));
				lblNewLabel.setIcon(new ImageIcon(newImage));
			}
		});
		slider_op.setValue(2);
		slider_op.setMaximum(6);
		slider_op.setBounds(359, 11, 75, 25);
		frmjavaSwing.getContentPane().add(slider_op);
		
		JLabel lblNewLabel_1 = new JLabel("op");
		lblNewLabel_1.setBounds(338, 10, 25, 15);
	...
	public Mat morphy(Mat source,int ksize,int shape,int op){
