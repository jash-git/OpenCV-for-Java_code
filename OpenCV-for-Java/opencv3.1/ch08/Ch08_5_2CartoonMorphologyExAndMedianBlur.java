...
public class Ch08_5_2CartoonMorphologyExAndMedianBlur {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch08_5_2CartoonMorphologyExAndMedianBlur window = new Ch08_5_2CartoonMorphologyExAndMedianBlur();
	...
	private void initialize() {
		...
		slider_ks.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(BlurAndCanny(source2,slider_th1.getValue(),slider_th2.getValue(),slider_ks.getValue(),slider_Ksize.getValue(),slider_OP.getValue()));
		...
		slider_Gamma.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				Mat layer2=BlurAndCanny(source2,slider_th1.getValue(),slider_th2.getValue(),slider_ks.getValue(),slider_Ksize.getValue(),slider_OP.getValue());
	...
	public Mat BlurAndCanny(Mat source,double threshold1,double threshold2,int GaussianKernelSize,int ksize,int op){
		 Mat destination=new Mat(source.rows(),source.cols(),source.type());
		//Imgproc.GaussianBlur(source, destination,new Size(GaussianKernelSize,GaussianKernelSize),60,110);
		...
		// Imgproc.GaussianBlur(destination, destination,new Size(GaussianKernelSize,GaussianKernelSize),30,10);
		 ...
		 Mat destination3=new Mat(destination.rows(),destination.cols(),destination.type());
		...
		// System.out.println("destination2="+destination2.channels());
		 ...
		 Mat element = Imgproc.getStructuringElement(0,new Size(2*ksize + 1, 2*ksize+1));
