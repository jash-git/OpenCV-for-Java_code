...
public class Ch10_21_1FindBrokenByHoughLinesP {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ch10_21_1FindBrokenByHoughLinesP window = new Ch10_21_1FindBrokenByHoughLinesP();
	 ...
	 */
	public Ch10_21_1FindBrokenByHoughLinesP() {
		initialize();
	}
	...
	private void initialize() {
		final Mat source = Imgcodecs.imread("C://opencv3.1//samples//IMAG0424.jpg");
		BufferedImage image=matToBufferedImage(source);
		
		frmjavaSwing = new JFrame();
		frmjavaSwing.setTitle("opencv 使用霍夫線變換找出有斷裂線");
		...
		slider_shape.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(HoughLine(slider_ksize.getValue(),slider_shape.getValue(),slider_op.getValue()));
	...
	public Mat HoughLine(int threshold,double minLineLength,double maxLineGap){
		...
		Mat src = Imgcodecs.imread("C://opencv3.1//samples//IMAG0424.jpg");
		Mat HSVprocess=matConvertTo();
		 ...
		 Imgproc.HoughLinesP(destination, lines, 10, Math.PI/180, threshold, minLineLength, maxLineGap);
		 ...
		 System.out.println("====================================");
		    for (int x = 0; x < lines.cols(); x++) 
		    {
		          ...
		          System.out.println("line"+x+":"+getDistance(start,end));
