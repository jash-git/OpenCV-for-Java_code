...
public class Ch10_24_1FindRotatedRectangleIntersection {
	...
	public static void main(String[] args) {
	 ...
	 Imgproc.rectangle(image2d, new Point(100,100),new Point(250,250), new Scalar(255,0,0));
	
	//矩形2
	 RotatedRect rect2=new RotatedRect(new Point(200,200),new Size(160,60),0);
	 Imgproc.rectangle(image2d, new Point(120,170),new Point(280,230), new Scalar(0,255,0));
	 
	 Mat intersectingRegion=new Mat();
	 
	 //求交叉
	 int sumOfIntersection=Imgproc.rotatedRectangleIntersection(rect1, rect2, intersectingRegion);
	 ...
	 for(int i=0;i<intersectingRegion.rows();i++){
			Point tmpPoint=new Point(intersectingRegion.get(i, 0)[0],intersectingRegion.get(i, 0)[1]);
			Imgproc.circle(image2d, tmpPoint, 4, new Scalar(0,0,255));
		}
	 
	 Imgcodecs.imwrite("C://opencv3.1//samples//FindRotatedRectangleIntersection.jpg", image2d );		 
	}
